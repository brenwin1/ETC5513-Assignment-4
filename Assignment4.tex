...
To map the data, shape files were downloaded from Open Minneapolis website (an open data repository containing spatial geographic information on Minneapolis) to draw the neighbourhood outlines and plot the police use of force and stop data circle points. \autocite{neighbourhood}

Data was then wrangled to join with demographics information retrieved from \autocite{mncompass} containing estimates of the 5-year average neighbourhood population with 2014-2018 American Community survey, adjusted to according to Census neighbourhood boundaries. This allows filling each neighbourhood(polygon) on a scale of proportion of black in a neighbourhood.

In assessing the maps, Whites and Blacks comprised the majority composition while minority groups are categorised as others.
